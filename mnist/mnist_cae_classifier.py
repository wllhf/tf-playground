...
from ae.cae import cae_classifier
...
def main():
    ...
    samples, labels = trn
    nsamples, shape = samples.shape[0], samples.shape[1:]
    dim_out = labels.shape[1]
    iterations = ceil(nsamples / MINI_BATCH_SIZE)

    # graph
    x = tf.placeholder(tf.float32, shape=[None, shape[0], shape[1], shape[2]], name='input_layer')
    y = tf.placeholder(tf.float32, shape=[None, dim_out], name='target')
    model = cae_classifier(x, y, [16, 8, 8], [(3, 3), (3, 3), (3, 3)], [(2, 2), (2, 2), (2, 2)])
