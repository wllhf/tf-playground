...
def batch_norm(input_tensor, is_training=False, decay=0.999, variance_epsilon=0.001):
    ...
    mean = tf.get_variable(name='mean_variable', initializer=tf.constant(0.0, shape=[dim]), trainable=False)
    var = tf.get_variable(name='var_variable', initializer=tf.constant(0.0, shape=[dim]), trainable=False)
    beta = tf.get_variable(name='beta_variable', initializer=tf.constant(0.0, shape=[dim]))
    gamma = tf.get_variable(name='gamma_variable', initializer=tf.constant(1.0, shape=[dim]))

    if is_training:
        batch_mean, batch_var = tf.nn.moments(input_tensor, keep_dims=False)
...
def conv2d_layer_bn(input_tensor, fshape, nchannels, nfilters,
                    strides=[1, 1, 1, 1], act=tf.nn.relu, dropout=None, is_training=False,
                    padding='SAME', name='conv2d_layer_bn'):

    with tf.name_scope(name):
        W = weight_variable([fshape[0], fshape[1], nchannels, nfilters])
        z = tf.nn.conv2d(input_tensor, W, strides=strides, padding=padding)
        y = act(batch_norm(z, is_training))
        return tf.nn.dropout(y, dropout) if dropout is not None else y
